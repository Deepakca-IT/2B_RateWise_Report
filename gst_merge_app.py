...
if gstr2b_file:
    ...
    available_months = df2b['Month-Year'].dropna().unique().tolist()
    selected_month = st.selectbox("Select Month-Year to filter GSTR-2A upload", sorted(available_months))

    gstr2a_files = st.file_uploader("Upload GSTR-2A Excel File(s) for Selected Month", type="xlsx", accept_multiple_files=True)
